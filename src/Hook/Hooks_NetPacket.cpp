...
namespace Hooks_NetPacket_RichPresence {
    ...
    bool HandleRecv(const uint8* pBody, uint32 cbBody, const uint8* pHdr, uint32 cbHdr)
    {
        ...
        // so the game's "friend playing same game" checks see 480 vs its own
        // real AppId and silently hide/refuse co-op partners. Rewrite friend
        // entries 480 -> real AppId here, in the same inbound persona, so both
        // the client cache (and therefore the game) see a consistent world.
        //
        // NOTE: this runs BEFORE the self-entry early-out -- friend deltas
        // usually arrive WITHOUT a self entry, and gating on it would drop

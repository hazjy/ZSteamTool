...
**重要事件时间线**见 `../../EVENTS/EVENTS.md`（发生时间 / 具体情况 / 是否解决 / 处置，含未结事项速查）；本文件只承载当前架构与技术决策。
...
1. **manifest 清单重定向失效（固定 gid 不更新）**：与 9/9 起 CM 对清单下载码加服务器验证相互印证——**"必须用最新清单，老清单不认"**。⚠️ 直接影响：**投喂 depotcache 旧 gid 清单在 Beta 上不再被采用**（须与 appinfo 当前 gid 一致且配最新码）；正式版 9/9 实测投喂仍可用（Sifu 成功），Beta 是否同步收口待验。
...
- 现象→机制链已闭环：**Beta 的"清单重定向失效" = 9/9 服务器验证收口的客户端侧正式化**——服务端+客户端双端收口，投喂路线的存活条件从"有清单文件"升级为"**最新 gid 清单 + 有效会话码**"（正式版暂只要求前者）。

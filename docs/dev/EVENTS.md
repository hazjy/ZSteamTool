...
- **结论修正**：9/9「跨会话/第三方码一律 401」的**前提错了**——真实条件是**码必须由有效 CM 会话签发**（群友服务疑似持有效会话池现场签发）；**有效码 = CDN 认，且 workshop 同样可下**。被卡死的不是"码绑定"，而是"有效码来源"。
- **意义**：请求码路线（第三方码服务）**复活**——最新清单可直接经"码服务+CDN"拉取（含工坊），**替代/补充清单投喂**；配合 chunk（无码）+ depot key（Sudama/内核库），**完整下载链无拥有权可行**。
- **处置/下一步**：① 换 `d:\steam\config\lua\manifest.lua` 为群友实现（备份短路版），内核自动注入有效码，实测一例无缓存清单游戏（如 2001760）验证全链；② 隧道域名稳定性待观察（漂移时需更新 lua）；③ GUI 侧把"码服务"纳入清单源体系（可选）。**短路策略调整**：对有效源启用注入，无效源（20770407 等）仍短路。

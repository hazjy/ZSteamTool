...
- **Steam 客户端大更新预警（Beta #1788989629）**：客户端内部重构，Beta 通道强制更新；请勿手动切换 Beta 通道，正式版当前链路可用（详见 `docs/dev/DEV-NOTES.md §9`）；
- **下载依赖清单投喂**：服务器对非拥有账号的清单请求码签发受限，新游戏下载需将清单文件放入 `steam\depotcache\`（详见 `EVENTS/02-下载与请求码.md`）；

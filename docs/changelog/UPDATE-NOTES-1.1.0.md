...
- **Steam 客户端大更新预警（Beta #1788989629）** 仍在：请勿手动切换 Beta 通道，正式版当前链路可用（适配要点见 `docs/dev/UPSTREAM-SYNC.md` 的「Steam 客户端大更新」一节）。
- **下载**：请求码收口环境下正路仍是**清单投喂 depotcache**；第三方码源可恢复部分游戏下载，属外部依赖，稳定性与可用性自担。
